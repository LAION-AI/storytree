...
#
# Per film it runs the full T1..T9 chain (max 5 traces per step) and then
# the 3-judge panel over every generated trace. Both stages skip tids that
# already exist, so re-running is safe and resumes where it stopped.
#
...
HF_DIR=""; OUT_DIR=""; FILMS=""; MAX_FILMS=""
while [ $# -gt 0 ]; do
  case "$1" in
...
echo "films: ${#SLUGS[@]}  per_layer: $PER_LAYER  model: $ZEN_MODEL"
echo "out: $OUT_DIR"
