...
Two rules about paths that decide whether an op lands:

1. `add` writes ONE key into a container that already exists. It does not create
   the containers along the way. If `/entities/ch-02/profile/backstory` exists
   but `b04a` does not, then
       add /entities/ch-02/profile/backstory/b04a/text
   is two operations, not one — first add `b04a` with its whole object as the
   value, then nothing further is needed. Prefer adding the complete object in a
   single op over walking down into it.
2. Never auto-create a path through an array. If a pointer passes through a list
   index, that container must already exist; say explicitly what you mean.

Emitting one op per violation is fine and preferable to one clever op that fixes
several — ops are applied independently, so a malformed one no longer discards
the rest, but it does lose its own fix.
